...
function DiffBadge({ d }: { d: string }) {
  ...
  return <span style={{ background: map[d] || "#888", color: "#fff", borderRadius: 5, padding: "3px 10px", fontSize: 13, fontWeight: 700, letterSpacing: 0.5, whiteSpace: "nowrap" }}>{d}</span>;
}
  ...
  return (
    <div>
      ...
      <div style={{ borderRadius: 14, overflow: "hidden", border: `1px solid ${border}` }}>
        {paginated.length === 0 && (
          <div style={{ padding: "36px 20px", textAlign: "center", color: sub, fontSize: 17 }}>
            ...
            <div key={w.word}
              ...
              {isExp && (
                <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px dashed ${border}` }}>
                  ...
                  <div style={{ color: sub, fontSize: 15, lineHeight: 1.5 }}>{w.exampleTranslation}</div>
      ...
      {totalPages > 1 && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 22, flexWrap: "wrap" }}>
          ...
          {pageNumbers.map((n, i) =>
            n === "…"
              ? <span key={`e${i}`} style={{ color: sub, fontSize: 18, padding: "0 2px" }}>…</span>
              : <button key={n} onClick={() => goPage(n as number)}
                  style={btn(page === n ? accent : (dark ? "#2a2a3e" : "#ede9ff"), page === n ? "#fff" : (dark ? "#c4b8ff" : accent), { padding: "11px 0", minWidth: 46, fontSize: 16 })}>
                  {n}
                </button>
          )}
  ...
  return (
    <div style={{ maxWidth: 500, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 16 }}>
        ...
        <button onClick={() => setFlipped(f => !f)} style={btn(dark ? "#383858" : "#ede9ff", dark ? "#c4b8ff" : accent, { padding: "14px", fontSize: 16 })}>
          {flipped ? "Hide" : "Flip"}
        </button>
    ...
    <div style={{ maxWidth: 500, margin: "0 auto" }}>
      ...
      {selected !== null && (
        <div style={{ textAlign: "center" }}>
          <div style={{ color: sub, fontSize: 15, marginBottom: 16, fontStyle: "italic", lineHeight: 1.6 }}>
            "{current.example}"<br />{current.exampleTranslation}
          </div>
          <button onClick={pickQuestion} style={btn(accent, "#fff", { width: "100%", padding: "16px", fontSize: 18 })}>
            Next question →
          </button>
        </div>
      )}
      ...
      <header style={{ background: navBg, borderBottom: `1px solid ${border}`, padding: "0 16px", position: "sticky", top: 0, zIndex: 100, boxShadow: dark ? "0 2px 16px #00000055" : "0 2px 16px #c4b5fd22" }}>
        <div style={{ maxWidth: 600, margin: "0 auto", display: "flex", alignItems: "center", height: 60 }}>
          ...
          <button onClick={toggleDark}
            style={{ background: dark ? "#383858" : "#ede9ff", border: "none", borderRadius: 20, padding: "8px 14px", cursor: "pointer", color: dark ? "#c4b8ff" : accent, display: "flex", alignItems: "center", WebkitTapHighlightColor: "transparent" }}>
